const tabsList = [
  ...
  class AppStore extends Component {
    state = {updateApplist: tabsList[0].tabId, searchInput: ''}
    selectItem = tabId => {
      this.setState({updateApplist: tabId})
    }
    filteredApps = () => {
      const {updateApplist} = this.state
      const filteredApps = appsList.filter(
        eachApp => eachApp.category === updateApplist,
      )
      return filteredApps
    }
    onChangeInput = event => {
    ...
    render() {
      let searchlist
      const {searchInput, updateApplist} = this.state
      const filteredApps = this.filteredApps()
      if (searchInput !== '') {
        searchlist = filteredApps.filter(eachlist =>
          eachlist.appName.toLowerCase().includes(searchInput.toLowerCase()),
        )
      } else {
        searchlist = filteredApps
      }
      return (
            ...
            <ul className="unordered-container">
              {tabsList.map(eachItem => (
                <TabItem
                  ...
                  activeStyle={eachItem.tabId === updateApplist}
                />
              ))}
            </ul>
            <ul className="unordered-container1">
              {searchlist.map(eachApp => (
